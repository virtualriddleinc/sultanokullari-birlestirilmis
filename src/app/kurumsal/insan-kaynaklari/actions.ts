...
const slugs = new Set(branches.map((b) => b.slug));

const ikSchema = z.object({
  fullName: z.string().trim().min(2),
  email: z.string().trim().email(),
  phone: z.string().trim().min(10),
  branchSlug: z.string().transform((s) => {
...
export async function submitIkApplication(
  ...
  const parsed = ikSchema.safeParse({
  ...
  return {
    ok: true,
    message:
      "Başvurunuz kaydedildi (demo). Üretimde dosya yükleme ve e-posta bildirimi eklenebilir.",
  };
}
